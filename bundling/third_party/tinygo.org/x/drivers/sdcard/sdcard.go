...
func (d Device) cmd(cmd byte, arg uint32, crc byte) byte {
	...
	// Reuse cmdbuf[0:1] (set to 0xFF) instead of allocating []byte{0xFF}
	// per iteration; without an SD card the loop runs 65535 times and the
	// resulting GC pressure crashes the runtime on small-RAM targets.
	d.cmdbuf[0] = 0xFF
	for i := 0; i < 0xFFFF; i++ {
		d.bus.Tx(d.cmdbuf[:1], d.tokenbuf)
